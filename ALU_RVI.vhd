...
ARCHITECTURE rtl OF alu_rvi IS
  ...
  signal  xorarg1, xorarg2, xorres, xor_rd          :  std_logic_vector(xlen-1 downto 0);
...
---------------------------------------------------------------------sub, subw, slt, sltu, blt, bqe, bltu, bqeu----------------------
l32_sub:if xlen = 32 generate
 
with  alu_rvi_instr.cop select
subarg1   <= '0' & alu_rvi_instr.rs1_value  when sub | slt | bltu,
             '0' & alu_rvi_instr.rs2_value  when bgeu,
             s1 & alu_rvi_instr.rs1_value(xlen - 1 downto 0) when slt | blt,
             s2 & alu_rvi_instr.rs2_value(xlen - 1 downto 0) when bge,
             (others => '0') when others;
			 
with  alu_rvi_instr.cop select
subarg2   <= '0' & alu_rvi_instr.rs2_value  when sub | slt | bltu,
             '0' & alu_rvi_instr.rs1_value  when bgeu,
             s2 & alu_rvi_instr.rs2_value(xlen - 1 downto 0) when slt | blt,
             s1 & alu_rvi_instr.rs1_value(xlen - 1 downto 0) when bge,
             (others => '0') when others;
...
subarg2   <= '0' & alu_rvi_instr.rs2_value  when sub | slt | bltu,
             '0' & alu_rvi_instr.rs1_value  when bgeu,
			 '0' & zero(31 downto 0) & alu_rvi_instr.rs2_value(31 downto 0) when subw,
             s2 & alu_rvi_instr.rs2_value(xlen - 1 downto 0) when slt | blt,
             s1 & alu_rvi_instr.rs1_value(xlen - 1 downto 0) when bge,
             (others => '0') when others;
...
l32_sub_rd:if xlen = 32 generate
   ...
   sub_rd <= subres(xlen-1 downto 0)  when sub,
             zero(xlen-1 downto 1) &  subres(xlen) when slt,
...
with  alu_rvi_instr.cop select
branch_state <= subres(xlen) when bltu,
                not subres(xlen) when bgeu,
                (s1 and (s1 xor s2)) or (sr and not(s1 xor s2)) when blt,
		        not ((s1 and (s1 xor s2)) or (sr and not(s1 xor s2))) when bge,
                '0' when others;
...
or_rd  <= orarg1 or orarg2;

---------------------------------------------------------------------xori(xor), beq, bne---------------------------------------------              
with  alu_rvi_instr.cop select
xorarg1 <= alu_rvi_instr.rs1_value  when xori | beq | bne,
           (others => '0') when others;
			 
with  alu_rvi_instr.cop select 
xorarg2 <= alu_rvi_instr.rs2_value  when xori | beq | bne,
           (others => '0') when others;

xorres  <= xorarg1 xor xorarg2;
 
with  alu_rvi_instr.cop select 
xor_rd  <= xorres  when xori,
           (others => '0') when others;
		   
with  alu_rvi_instr.cop select 
branch_state <= REDUCTION_OR(xorres)  when  bne,
                not REDUCTION_OR(xorres)  when  beq,
                '0' when others;
...
with  alu_rvi_instr.cop select 
branch <= '1' when beq | bne | blt | bqe | bltu | bqeu,
          '0' when others; 
